...
class TRP_IN_SP_Slug_Manager {
    ...
    protected $sanitized_request_uri;
...
//            }
     ...
     *
     * We do this due to redirect_canonical, hooked on template redirect - 10.
     * Redirect canonical compares (under certain circumstances) the REQUEST_URI with URL received through get_permalink.
     * Since we are hooked inside get_permalink, it will return the translated URL and redirect_canonical will enter a redirect loop due to the discrepancy.
     *
     * @hooked template_redirect - 9
     * @return void
     */
    public function revert_request_uri(){
        if ( !empty( $this->sanitized_request_uri ) && isset( $_SERVER['REQUEST_URI'] ) && $this->sanitized_request_uri !== $_SERVER['REQUEST_URI'] )
            $_SERVER['REQUEST_URI'] = $this->sanitized_request_uri;
    }
    ...
    public function translate_request_uri() {
        ...
        $this->sanitized_request_uri = sanitize_url( $_SERVER['REQUEST_URI'] );/* phpcs:ignore */ /* sanitized with sanitize_url() */
        ...
        $new_request_uri = sanitize_url(
            $this->get_slug_translated_url_for_language(
                $this->sanitized_request_uri,
                $this->sanitized_request_uri,
                $this->settings['default-language']
            )
        );

        do_action('trp_translated_request_uri', $new_request_uri, $this->sanitized_request_uri );

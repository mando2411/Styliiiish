...
Upload the plugin files to the `/wp-content/plugins/ai-alt-text-generator` directory, or install the plugin through the WordPress plugins screen directly.
...
* rename folder from `ai-alt-text-generator` to `react-settings`
* rename all files from `ai-alt-text-generator` to `react-settings` PHP,JS and CSS
* change `ai_alt_text_generator` to `react_settings`
* change `ai-alt-text-generator` to `react-settings`
* change `Ai_Alt_Text_Generator` to `React_Settings`
* change `AI_ALT_TEXT_GENERATOR` to `REACT_SETTINGS`
